...
const Contracts = () => {
  ...
  const loadContracts = async () => {
    ...
    setError(undefined);
    try {
      const payload = await window.gnoman.invoke<ContractRecord[]>('contract:list');
  ...
  useEffect(() => {
    const loadWallets = async () => {
      try {
        const payload = await window.gnoman.invoke<WalletSummary[]>('wallet:list');
        setWallets(payload);
        setTxForm((prev) => ({
          ...prev,
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const record = await window.gnoman.invoke<ContractRecord>('contract:add', {
  ...
  const deleteContract = async (id: string) => {
    setMessage(undefined);
    try {
      await window.gnoman.invoke('contract:remove', { id });
